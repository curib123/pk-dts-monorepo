...
import { AreaReference, AssetReference, DocumentFormValue, DocumentTypeValue, DocumentUserSummary, LocationReference, SequenceReference, SoftcopyCategoryReference, SpecificReference, WorkflowPlanStepValue } from '../../documents.types';
    ...
    template: `
        ...
        >
            ...
            <div class="grid gap-4 pt-2 md:grid-cols-2">
                ...
                <div class="field softcopy-upload md:col-span-2" *ngIf="!isHardcopy() && (form.direct_create || requiresProposedSoftcopy())">
                    <label for="softcopy-file">{{ form.direct_create ? 'Controlled softcopy file' : 'Proposed softcopy file' }} <span *ngIf="mode === 'create' || form.direct_create" class="text-red-500">*</span></label>
                    ...
                    <small class="field-note" *ngIf="!analyzingFile">{{ fileAnalysisMessage || (form.direct_create ? 'Required for direct creation as a controlled copy.' : 'Upload the exact file approvers should review. It remains a pending proposal and becomes the controlled/current revision only after final approval.') }}</small>
                    <small class="field-note selected-file" *ngIf="form.initial_file"><i class="pi pi-paperclip"></i> Selected: {{ form.initial_file.name }}</small>
                    <small *ngIf="submitted && mode === 'create' && !form.direct_create && requiresProposedSoftcopy() && !form.initial_file">A proposed Softcopy file is required before submitting this request.</small>
                </div>

                <div class="field" *ngIf="!isHardcopy() && (requiresProposedSoftcopy() || form.direct_create)">
                    <label for="initial-revision-number">{{ form.direct_create ? 'Initial revision number' : 'Proposed revision number' }}</label>
                ...
                <div class="field softcopy-upload md:col-span-2" *ngIf="!isHardcopy()">
                    ...
                    <small class="field-note">Uploaded separately from the proposed revision file. These remain request attachments and never become the approved controlled file automatically.</small>
                ...
                <div class="field" *ngIf="!isHardcopy()">
                    <label for="page-number">Page Number <span *ngIf="requiresProposedSoftcopy() || form.direct_create" class="text-red-500">*</span></label>
                    <input id="page-number" pInputText [(ngModel)]="form.page_number" class="w-full" placeholder="1-5" />
                    <small *ngIf="submitted && (requiresProposedSoftcopy() || form.direct_create) && !form.page_number?.trim()">Page Number is required before the proposed file enters approval.</small>
                </div>
                <div class="field" *ngIf="!isHardcopy() && (isRevisionAction() || form.direct_create)">
                ...
                <div class="field" *ngIf="!isHardcopy() && (requiresProposedSoftcopy() || form.direct_create)">
                    <label for="new-effective-date">{{ isRevisionAction() ? 'New Effective Date' : 'Effective Date' }} <span class="text-red-500">*</span></label>
                    <input id="new-effective-date" type="date" [(ngModel)]="form.new_effective_date" class="select-field" />
                    <small *ngIf="submitted && !form.new_effective_date">Effective Date is required before the proposed file enters approval.</small>
                </div>
                <div class="field md:col-span-2" *ngIf="!isHardcopy() && (requiresProposedSoftcopy() || form.direct_create)">
                ...
                <div class="field" *ngIf="isHardcopy()">
                    ...
                    <ng-container *ngIf="assets.length > searchableThreshold; else defaultAssetSelect">
                        <app-searchable-dropdown inputId="asset-id" [value]="form.asset_id" [options]="assetOptions" placeholder="No asset number" [disabled]="saving || referenceLoading" [loading]="referenceLoading" [clearValue]="''" (valueChange)="selectAsset(normalizeSelectValue($event))" />
                    </ng-container>
                ...
                <div class="field" *ngIf="isHardcopy()">
                    ...
                    <ng-container *ngIf="areas.length > searchableThreshold; else defaultAreaSelect">
                        <app-searchable-dropdown inputId="area-id" [value]="form.area_id" [options]="areaOptions" placeholder="Select area" [disabled]="saving || referenceLoading" [loading]="referenceLoading" [invalid]="submitted && isHardcopy() && !form.area_id" [required]="true" [clearValue]="''" (valueChange)="selectArea(normalizeSelectValue($event))" />
                    </ng-container>
                ...
                <div class="field" *ngIf="isHardcopy()">
                    ...
                    <ng-container *ngIf="specifics.length > searchableThreshold; else defaultSpecificSelect">
                        <app-searchable-dropdown inputId="specific-id" [value]="form.specific_id" [options]="specificOptions" placeholder="No specific" [disabled]="saving || referenceLoading" [loading]="referenceLoading" [clearValue]="''" (valueChange)="selectSpecific(normalizeSelectValue($event))" />
                    </ng-container>
                ...
                <div class="field" *ngIf="isHardcopy()">
                    ...
                    <ng-container *ngIf="locations.length > searchableThreshold; else defaultLocationSelect">
                        <app-searchable-dropdown inputId="location-id" [value]="form.location_id" [options]="locationOptions" placeholder="Select location" [disabled]="saving || referenceLoading" [loading]="referenceLoading" [invalid]="submitted && isHardcopy() && !form.location_id" [required]="true" [clearValue]="''" (valueChange)="selectLocation(normalizeSelectValue($event))" />
                    </ng-container>
                ...
                <div class="field" *ngIf="isHardcopy()">
                    ...
                    <ng-container *ngIf="sequences.length > searchableThreshold; else defaultSequenceSelect">
                        <app-searchable-dropdown inputId="sequence-id" [value]="form.sequence_id" [options]="sequenceOptions" placeholder="No sequence" [disabled]="saving || referenceLoading" [loading]="referenceLoading" [clearValue]="''" (valueChange)="form.sequence_id = normalizeSelectValue($event)" />
                    </ng-container>
                ...
                <div *ngIf="isHardcopy()" class="field md:col-span-2 retention-panel">
                    ...
                    <div *ngIf="form.retention_enabled" class="grid gap-4 md:grid-cols-2">
                        <div class="field"><label for="retention-start-date">Retention starts</label><input id="retention-start-date" type="date" [(ngModel)]="form.retention_start_date" class="select-field" [disabled]="saving" /></div>
                        <div class="field"><label for="retention-end-date">Retention ends</label><input id="retention-end-date" type="date" [(ngModel)]="form.retention_end_date" class="select-field" [disabled]="saving" /></div>
                    </div>
            ...
            <ng-template pTemplate="footer">
                ...
                <p-button [label]="mode === 'create' ? (isHardcopy() ? 'Send for Approval' : 'Submit Request') : (isHardcopy() ? 'Save and Send for Approval' : 'Save and Submit Request')" icon="pi pi-check" [loading]="saving" [disabled]="analyzingFile" (onClick)="submit('SUBMIT')" />
            </ng-template>
        </p-dialog>
    `,
    styles: [
        `
            ...
            .field small { color: #dc2626; font-size: 0.8rem; font-weight: 600; }
            .field .field-note { color: #64748b; font-weight: 500; }
            .selected-file { display: inline-flex; align-items: center; gap: .35rem; }
            ...
            .retention-panel { border: 1px solid #e2e8f0; border-radius: 1rem; background: #f8fafc; padding: 1rem; }
            .retention-toggle { display: flex; align-items: flex-start; gap: 0.7rem; cursor: pointer; }
            .retention-toggle input { margin-top: 0.2rem; accent-color: #991b1b; }
            .retention-toggle span { display: grid; gap: 0.25rem; }
            .retention-toggle small { color: #64748b; font-weight: 500; line-height: 1.45; }
            .field-error { color: #dc2626; font-size: 0.8rem; font-weight: 600; }
            .file-field { width: 100%; border: 1px dashed #94a3b8; border-radius: 0.85rem; padding: 0.75rem; background: #f8fafc; }
            ...
            .field .current-user-note { color: #475569; font-weight: 500; }
            .select-field { min-height: 2.75rem; width: 100%; border-radius: 0.85rem; border: 1px solid #cbd5e1; background: #ffffff; padding: 0.75rem 0.9rem; color: #0f172a; outline: none; }
            .select-field:focus { border-color: #0f172a; }
            :host ::ng-deep .p-dialog { border-radius: 1.5rem; overflow: hidden; }
            :host ::ng-deep .p-dialog .p-dialog-header { padding: 1.35rem 1.5rem 1rem; border-bottom: 1px solid rgba(148, 163, 184, 0.15); background: radial-gradient(circle at top left, rgba(220, 38, 38, 0.08), transparent 35%), linear-gradient(180deg, #ffffff 0%, #f8fafc 100%); }
...
export class DocumentFormDialogComponent implements OnChanges {
    ...
    @Input() form: DocumentFormValue = {
        document_number: '', document_title: '', document_type: 'HARDCOPY', action: 'DRAFT', requester_type: 'CURRENT_USER', requested_by_name: '',
        asset_id: '', area_id: '', specific_id: '', location_id: '', sequence_id: '', softcopy_category_id: '', initial_revision_number: '', initial_file: null,
        attached_scan_files: [], assigned_user_ids: [], retention_enabled: false, retention_start_date: '', retention_end_date: '', request_date: '', business_document_type: 'Forms',
        action_requested: 'CREATE', series_number: '', page_number: '', department: '', from_party: '', to_party: '', reason_for_change: 'Improvement', brief_description: '', proposed_change: '',
        revision_level_from: '', revision_level_to: '', previous_effective_date: '', new_effective_date: '', workflow_name: '', workflow_version: 1, workflow_steps: []
    };
    ...
    submit(action: 'DRAFT' | 'SUBMIT') {
        ...
        if (this.mode === 'create' && this.canAssignUsers && !this.form.workflow_version_id && !this.form.workflow_steps.length) this.applyWorkflowPreset('RECOMMENDED');
        this.form.document_title = this.form.document_title.trim().toUpperCase();
        if (!this.form.document_title.trim()) return;
        if (this.isHardcopy() && (!this.form.area_id || !this.form.location_id)) return;
        if (this.form.direct_create && (!this.form.direct_creation_reason?.trim() || !this.form.initial_file)) return;
        if (this.isHardcopy() && this.form.retention_enabled && (!this.form.retention_start_date || !this.form.retention_end_date)) return;
        ...
        if (!this.isHardcopy() && (this.requiresProposedSoftcopy() || this.form.direct_create) && action === 'SUBMIT') {
            if (this.mode === 'create' && !this.form.initial_file) return;
            if (!this.form.page_number?.trim() || !this.form.new_effective_date) return;
        }
    ...
    isHardcopy() { return this.form.document_type === 'HARDCOPY'; }
    requiresProposedSoftcopy() { return ['CREATE', 'REVISE', 'CREATE_REVISE'].includes(this.form.action_requested || 'CREATE'); }

    selectInitialFile(event: Event) {
        const input = event.target as HTMLInputElement;
        const file = input.files?.[0] ?? null;
        this.form.initial_file = file;
        if (file) this.form.attached_scan_files = this.form.attached_scan_files.filter((attachment) => !this.isSameFile(attachment, file));
        if (!file) return;

        const baseName = file.name.replace(/\.[^.]+$/, '').trim();
        const revisionMatch = baseName.match(/\brev(?:ision)?[\s._-]*([A-Z]?\d{1,4})\b/i);
        if (!this.form.initial_revision_number.trim() && revisionMatch) this.form.initial_revision_number = revisionMatch[1];
        if (!this.form.document_title.trim()) {
            const withoutMetadata = baseName.replace(/(?:^|[\s_-])rev(?:ision)?[\s._-]*[A-Z]?\d{1,4}(?:$|[\s_-])/i, ' ').replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
            if (withoutMetadata) this.form.document_title = withoutMetadata.toUpperCase();
        }
    ...
    onActionRequestedChange() {
        ...
        if (!this.requiresProposedSoftcopy() && !this.form.direct_create) {
    ...
    isRevisionAction() { return this.form.action_requested === 'REVISE' || this.form.action_requested === 'CREATE_REVISE'; }
    ...
    selectedPublishedWorkflow() { return this.publishedWorkflows.find((workflow) => workflow.workflow_version_id === this.form.workflow_version_id); }
    ...
    applyWorkflowPreset(preset: string) {
        ...
        const resolvedPreset = preset === 'RECOMMENDED' ? this.isHardcopy() ? 'DIRECT_HARDCOPY' : this.form.action_requested === 'CANCELLATION' ? 'CANCELLATION_SOFTCOPY' : 'STANDARD_SOFTCOPY' : preset;
        const stages: WorkflowPlanStepValue['stage'][] = resolvedPreset === 'STANDARD_SOFTCOPY' ? ['NOTED_BY', 'PLANT_MANAGER', 'DOCUMENT_CONTROLLER_ADMIN'] : resolvedPreset === 'CANCELLATION_SOFTCOPY' ? ['NOTED_BY', 'DOCUMENT_CONTROLLER_ADMIN'] : resolvedPreset === 'DIRECT_CONTROLLER' ? ['DOCUMENT_CONTROLLER_ADMIN'] : ['HARDCOPY_APPROVAL'];
        ...
        this.form.workflow_name = resolvedPreset === 'STANDARD_SOFTCOPY' ? 'Standard Softcopy Approval' : resolvedPreset === 'CANCELLATION_SOFTCOPY' ? 'Softcopy Cancellation Approval' : resolvedPreset === 'DIRECT_CONTROLLER' ? 'Direct Document Controller Approval' : 'Direct Hardcopy Approval';
        this.form.workflow_version = 1;
    }

    workflowStageLabel(stage: WorkflowPlanStepValue['stage']) {
        return ({ NOTED_BY: 'Leader / Noted By', PLANT_MANAGER: 'Plant Manager Approval', DOCUMENT_CONTROLLER_ADMIN: 'Document Controller Approval', HARDCOPY_APPROVAL: 'Hardcopy Approval' } as const)[stage];
    }
    ...
    approverOptions(step: WorkflowPlanStepValue): SearchableDropdownOption[] {
        return this.users.filter((user) => this.isLikelyWorkflowApprover(user, step.stage)).map((user) => ({ label: [this.fullName(user) || user.username || user.user_id, user.position_title, user.role?.role_name].filter(Boolean).join(' · '), value: user.user_id }));
    }
    ...
    selectScanFiles(event: Event) {
        ...
        this.form.attached_scan_files = Array.from(input.files ?? []).filter((file) => !revisionFile || !this.isSameFile(file, revisionFile)).slice(0, 10);
    }
    private isSameFile(left: File, right: File) { return left.name.trim().toLowerCase() === right.name.trim().toLowerCase() && left.size === right.size; }
    trackSoftcopyCategory = (_index: number, category: SoftcopyCategoryReference) => category.softcopy_category_id;
    get assetOptions(): SearchableDropdownOption[] { return this.filteredAssets.map((asset) => ({ label: asset.asset_number, value: asset.asset_id })); }
    get userOptions(): SearchableDropdownOption[] { return this.users.map((user) => ({ label: this.fullName(user) || user.username || user.user_id, value: user.user_id })); }
    get areaOptions(): SearchableDropdownOption[] { return this.areas.map((area) => ({ label: area.area_name, value: area.area_id })); }
    get specificOptions(): SearchableDropdownOption[] { return this.filteredSpecifics.map((specific) => ({ label: specific.specific_name, value: specific.specific_id })); }
    get locationOptions(): SearchableDropdownOption[] { return this.filteredLocations.map((location) => ({ label: location.location_name, value: location.location_id })); }
    get sequenceOptions(): SearchableDropdownOption[] { return this.sequences.map((sequence) => ({ label: sequence.sequence_code, value: sequence.sequence_id })); }
    ...
    cancel() { this.cancelClick.emit(); this.close(); }
    handleHide() { this.close(); }
    private close() { this.visible = false; this.visibleChange.emit(false); }
    normalizeSelectValue(value: SearchableDropdownValue) { return value === null || value === undefined ? '' : String(value); }
    ...
    selectLocation(value: string) {
        ...
        else if (location?.specific_id) { this.selectSpecific(location.specific_id); this.form.asset_id = ''; }
        this.form.location_id = value || '';
    }
