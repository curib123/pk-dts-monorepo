...
export const panelRoutes: Routes = [
    {
        ...
        children: [
            ...
            { path: 'backup-restore', loadComponent: () => import('./pages/backup-restore/backup-restore.page').then((m) => m.BackupRestorePage), data: { eyebrow: 'System administration', title: 'Backup, Restore and Reset', subtitle: 'Create snapshots, restore history, and run guarded reset actions.', permissions: PANEL_ROUTE_PERMISSIONS.backupRestore } },
            { path: 'audit-logs', loadComponent: () => import('./pages/audit-logs/audit-logs.page').then((m) => m.AuditLogsPage), data: { eyebrow: 'System administration', title: 'Audit and Activity Logs', subtitle: 'Trace authenticated document and system actions without loading unnecessary record payloads.', permissions: PANEL_ROUTE_PERMISSIONS.auditLogs } },
