...
@Component({
    selector: 'app-approval-review-page', standalone: true, imports: [DocumentDetailDialogComponent, CommonModule, ButtonModule, DialogModule, InputTextModule, TableModule, DataViewSwitchComponent, RecordGridComponent, RecordCardComponent, ConfirmationDialogComponent, LoadingShimmerComponent],
    ...
    <ng-template pTemplate="body" let-item><tr><td><strong>{{item.document_type === 'HARDCOPY' ? item.document_title : (item.document_number || 'No document number')}}</strong><small>{{item.document_title}}</small></td><td><span class="stage-pill">{{stageLabel(item)}}</span></td><td>{{name(item.creator)}}</td><td>{{item.requested_by_name || name(item.requester)}}</td><td>{{item.updated_at || item.created_at | date:'medium'}}</td><td><input pInputText #remarks placeholder="Optional remarks" /><small *ngIf="canRequestRevision(item)">A reason is required to return.</small></td><td class="actions"><p-button label="View" icon="pi pi-eye" size="small" [outlined]="true" [loading]="viewLoadingId === item.document_id" (onClick)="viewDocument(item)" /><p-button *ngIf="item.status === 'Approved' && canComplete()" label="Complete / Release" icon="pi pi-send" size="small" severity="success" [disabled]="acting" (onClick)="openDecision(item,'complete',remarks.value)"/><ng-container *ngIf="item.status !== 'Approved'"><p-button *ngIf="canApprove(item)" label="Approve" size="small" [disabled]="acting" (onClick)="openDecision(item,'approve',remarks.value)"/><p-button *ngIf="canRequestRevision(item)" label="Return for revision" size="small" [outlined]="true" [disabled]="acting" (onClick)="openDecision(item,'request-revision',remarks.value)"/><p-button *ngIf="canReject(item)" label="Reject" size="small" severity="danger" [disabled]="acting" (onClick)="openDecision(item,'reject',remarks.value)"/></ng-container></td></tr></ng-template>
    ...
    <app-record-grid *ngIf="viewMode === 'grid'" [empty]="!requests().length && !loading" emptyTitle="No pending approvals" emptyMessage="No requests are awaiting approval.">
        <app-record-card *ngFor="let item of requests()" icon="pi pi-verified" eyebrow="Approval request" [title]="item.document_type === 'HARDCOPY' ? item.document_title : (item.document_number || 'No document number')" [subtitle]="item.document_title">
            <div record-details>
                ...
                <div class="wide"><span>Remarks</span><input pInputText #gridRemarks placeholder="Optional remarks" /><small *ngIf="canRequestRevision(item)">A reason is required to return.</small></div>
            </div>
            <div record-actions><p-button label="View" icon="pi pi-eye" size="small" [outlined]="true" [loading]="viewLoadingId === item.document_id" (onClick)="viewDocument(item)" />
                ...
                <ng-container *ngIf="item.status !== 'Approved'"><p-button *ngIf="canApprove(item)" label="Approve" size="small" [disabled]="acting" (onClick)="openDecision(item,'approve',gridRemarks.value)"/>
                <p-button *ngIf="canRequestRevision(item)" label="Return for revision" size="small" [outlined]="true" [disabled]="acting" (onClick)="openDecision(item,'request-revision',gridRemarks.value)"/>
                <p-button *ngIf="canReject(item)" label="Reject" size="small" severity="danger" [disabled]="acting" (onClick)="openDecision(item,'reject',gridRemarks.value)"/></ng-container>
            </div>
        </app-record-card>
    ...
    <p-dialog [(visible)]="returnRemarkVisible" [modal]="true" [closable]="false" [draggable]="false" [resizable]="false" header="Return for revision" styleClass="return-remark-dialog">
        <div class="return-remark-content">
            <p>Explain what needs to be corrected before this request is returned to the requester.</p>
            <label for="return-remark">Reason for return <span>*</span></label>
            <textarea id="return-remark" #returnRemark rows="5" maxlength="2000" autofocus placeholder="Enter the correction needed..."></textarea>
            <small>A reason is required and will be recorded with your name.</small>
            <div class="return-remark-actions">
                ...
                <p-button label="Return for revision" icon="pi pi-replay" [disabled]="acting || !returnRemark.value.trim()" (onClick)="submitReturnRemark(returnRemark.value)" />
            </div>
        </div>
    </p-dialog>
...
export class ApprovalReviewPage implements OnInit, OnDestroy {
    ...
    returnRemarkVisible = false;
    openDecision(item:DocumentSummary,action:'approve'|'request-revision'|'reject'|'complete',remarks:string){this.errorMessage.set('');if(action==='request-revision'){this.pendingDecision={item,action,remarks:''};this.returnRemarkVisible=true;this.decisionVisible=false;return;}this.pendingDecision={item,action,remarks};this.decisionVisible=true;}
    submitReturnRemark(remarks:string){const decision=this.pendingDecision;if(!decision||decision.action!=='request-revision'||!remarks.trim())return;decision.remarks=remarks.trim();this.returnRemarkVisible=false;this.confirmDecision();}
    confirmDecision(){const decision=this.pendingDecision;if(!decision||this.acting)return;this.acting=true;this.documents.workflowAction(decision.item.document_id,decision.action,decision.remarks).subscribe({next:()=>{this.requests.update(items=>items.filter(item=>item.document_id!==decision.item.document_id));this.acting=false;this.clearDecision();this.alerts.success('Request updated',`${decision.item.document_number || decision.item.document_title} was ${this.actionOutcome(decision.action)} successfully.`);this.load();},error:(error)=>{this.acting=false;const message=error?.error?.message;this.errorMessage.set(Array.isArray(message)?message.join(' '):message||`Unable to ${this.actionText(decision.action).toLowerCase()} this request.`);if(decision.action==='request-revision')this.returnRemarkVisible=true;}});}
    clearDecision(){this.pendingDecision=null;this.decisionVisible=false;this.returnRemarkVisible=false;}
